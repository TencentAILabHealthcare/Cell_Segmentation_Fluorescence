...
class COCOeval:
    ...
    def _prepare(self):
        ...
        for gt in gts:
            ...
            # gt['ignore'] = 'iscrowd' in gt and gt['iscrowd'] # wentaopan
    ...
    def summarize(self):
        ...
        def _summarizeDets():
            ...
            stats[8] = _summarize(0, iouThr=.3, maxDets=self.params.maxDets[2])
            stats[9] = _summarize(0, iouThr=.5, maxDets=self.params.maxDets[2])
            stats[10] = _summarize(0, iouThr=.75, maxDets=self.params.maxDets[2])
            stats[11] = _summarize(0, iouThr=.5, areaRng='small', maxDets=self.params.maxDets[2])
            stats[12] = _summarize(0, iouThr=.5, areaRng='medium', maxDets=self.params.maxDets[2])
            stats[13] = _summarize(0, iouThr=.5, areaRng='large', maxDets=self.params.maxDets[2])
            return stats
        def _summarizeKps():
            stats = np.zeros((10,))
...
class Params:
    ...
    def setDetParams(self):
        ...
        self.areaRng = [[0 ** 2, 1e5 ** 2], [0, 100], [100, 700], [700, 1e5 ** 2]]
