...
def image2coco(root, path_list):
    ...
    for image_path, mask_path in tqdm(path_list):
        ...
        mask = cv2.imread(os.path.join(root, mask_path), cv2.IMREAD_UNCHANGED)
...
def process_train():
    root = "/mnt/zihanwu/wentaopan/cell_instance_segmentation/data/conA"
    target = "/mnt/zihanwu/wentaopan/cell_instance_segmentation/data/conA/train_1.json"
    path_list = []
    source = "/mnt/zihanwu/wentaopan/cell_instance_segmentation/data/conA/1_size256_overlap64"
    ...
    for f in os.listdir(source):
        ...
        path_list.append([
            ...
            os.path.join(folder, f.replace("conA", "mask"))
        ])     
...
def process_val():
    root = "/mnt/zihanwu/wentaopan/cell_instance_segmentation/data/conA"
    target = "/mnt/zihanwu/wentaopan/cell_instance_segmentation/data/conA/val.json"
    path_list = []
    source = "/mnt/zihanwu/wentaopan/cell_instance_segmentation/data/conA/1_size256_overlap64"
    folder = os.path.basename(source)
    for f in os.listdir(source):
