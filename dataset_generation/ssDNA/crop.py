...
def process_train(source, dist, size, overlap): 
    ...
    for f in tqdm(os.listdir(source)):
        ...
        patches = crop(img, patch_size=size, overlap=overlap)
        for (h, w), patch_image, patch_mask in patches:
            image_name = "{}_{}_{}_h{}_w{}_{}.png".format(*f.split("_")[:3], h, w, "ssDNA")
            cv2.imwrite(os.path.join(dist, image_name), patch_image)



def process_val(source, dist, size, overlap): 
    os.makedirs(dist, exist_ok=True)
    for f in tqdm(os.listdir(source)):
        if "mask" in f:
            continue
        path = os.path.join(source, f)
        img = cv2.imread(path, cv2.IMREAD_UNCHANGED)
        msk = cv2.imread(os.path.join(source, f.replace("ssDNA", "ssDNA_mask")), cv2.IMREAD_UNCHANGED)
        patches = crop(img, mask=msk, patch_size=size, overlap=overlap)
        for (h, w), patch_image, patch_mask in patches:
            image_name = "{}_h{}_w{}_{}.png".format(*f.split("_")[:1], h, w, "ssDNA")
            cv2.imwrite(os.path.join(dist, image_name), patch_image)
            mask_name = "{}_h{}_w{}_{}.png".format(*f.split("_")[:1], h, w, "mask")
            cv2.imwrite(os.path.join(dist, mask_name), patch_mask)
    

if __name__ == "__main__":
    process_train(
        "/mnt/zihanwu/wentaopan/cell_instance_segmentation/data/ssDNA/1",
        "/mnt/zihanwu/wentaopan/cell_instance_segmentation/data/ssDNA/1_size256_overlap64",
        256,
        64,
    )
    process_val(
        "/mnt/zihanwu/wentaopan/cell_instance_segmentation/data/ssDNA/val",
        "/mnt/zihanwu/wentaopan/cell_instance_segmentation/data/ssDNA/val_size256_overlap64",
        256,
        64,
    )